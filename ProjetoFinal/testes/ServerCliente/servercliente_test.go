...
func TestIncluirCliente(t *testing.T){
	...
	reg.Nome = "Josue Da Silvaa Alencar"
	reg.Email = "jose@jose.com"
	reg.Telefone = "(62) 99494-9494"
...
func TestBuscarTodosClientes(t *testing.T){
	...
	mode.Tipo = ServidorClientes.ModoBusca_TODOS
	...
	if resp.Message.Error != 0 {
		t.Errorf("Falha ao buscar todos os clientes!\n" + resp.Message.Message)
	}

	fmt.Printf("\nTotal de %d Clientes\n-------------------------\n", len(resp.Clientes))

	for i := 0; i < len(resp.Clientes) ; i++ {
		fmt.Printf("Id: %d\nNome: %s\nEmail: %s\nTelefone: %s\nData Cadastro: %s\n-------------------------\n",
			 resp.Clientes[i].Id, resp.Clientes[i].Nome, resp.Clientes[i].Email, 
			 	resp.Clientes[i].Telefone, resp.Clientes[i].DataCadastro)
	}
}
